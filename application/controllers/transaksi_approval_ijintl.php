...
class Transaksi_approval_ijintl extends CI_Controller{
    ...
    $sQuery = "SELECT TOP $limit ".implode(",",$aColumns)."
		...
		$rResult = $this->m_trappijintl->getlist_ijintl($aColumns, $sWhere, $sOrder, $top, $limit, $parameter_login);
       
		$iTotal = 0;
        $rResultTotal = $this->m_trappijintl->getlist_ijintl_total($sIndexColumn, $parameter_login);
        $iTotal = $rResultTotal->num_rows();
		
		$iFilteredTotal = 0;
		$rResultTotalFiltered = $this->m_trappijintl->getlist_ijintl_filteredtotal($sIndexColumn, $parameter_login, $aColumns, $sWhere, $sOrder, $top, $limit);
		...
		foreach ($rResult->result() as $aRow) 
		{
			...
			$row = array();			
				...
				$is_pimpinan_approve_tl = $aRow->is_pimpinan_approve_tl;
				...
				*/
				...
				if ($is_pimpinan_approve_tl == 0)	
				{
					...
					$row[] = '<i class="ui-tooltip fa fa-check-square-o" title="Proses" style="font-size: 22px;color:#aa2222; cursor:pointer;" data-original-title="Proses" onclick="proses_approval_ijintl('."'".$aRow->idabsen."'".','."'".$parameter_login['level']."'".')"></i>&nbsp;
    ...
    $sQuery = "SELECT TOP $limit ".implode(",",$aColumns)."
		...
		foreach ($rResult->result() as $aRow) 
		{
			...
			$row = array();			
				...
				//	atur kolom status petugas
				$is_petugas_approve_tl = $aRow->is_petugas_approve_tl;
				...
				if ($is_petugas_approve_tl == 0)	
				{
					...
					$row[] = '<i class="ui-tooltip fa fa-check-square-o" title="Proses" style="font-size: 22px;color:#aa2222; cursor:pointer;" data-original-title="Proses" onclick="petugas_approval_ijintl('."'".$aRow->idabsen."'".','."'".$parameter_login['level']."'".', '."'".$aRow->is_pimpinan_approve_tl."'".')"></i>&nbsp;
					<i class="ui-tooltip fa fa-book" title="Detail" style="font-size: 22px;color:#2222aa;cursor:pointer;" data-original-title="Detail" onclick="#"></i>';
	...
	function list_approval_ijintl()
	{
		...
		$data['title']="<i class=\"fa fa-user fa-fw\"></i> Transaksi Approval Ijin Terlambat Datang";
		$data['subtitle']=" Transaksi Approval Ijin Terlambat Datang / Pegawai";
		$data['tabletitle']="Table Transaksi Approval Ijin Terlambat Datang";
		$data['navigasi']="<li><a href=\"#\"><i class=\"fa fa-dashboard\"></i> Home</a></li>
            <li><a href=\"#\">Transaksi</a></li>
            <li class=\"active\">Approval Ijin Terlambat Datang</li>";
			
		$this->load->view('header',$data);
		$this->load->view('leftsidebar');
		$this->load->view('transaksi_approval_ijintl/list_approval_ijintl',$data);
	...
	function proses_approval_ijintl()
	{
		...
		$data['list'] = $this->m_trappijintl->getBiodataMskaryawanByIdAbsen($idabsen, $level_user);
		$data['tabsen'] = $this->m_trappijintl->getTabsenByIdAbsen($idabsen);
		$data['path_attachment_tl'] = $this->config->item('path_attachment_tl');
		
		$this->load->view('transaksi_approval_ijintl/pimpinan_approval_ijintl.php',$data);
		
	}
	
	function petugas_approval_ijintl()
	{
		...
		$data['list'] = $this->m_trappijintl->getBiodataMskaryawanByIdAbsen($idabsen, $level_user);
		$data['tabsen'] = $this->m_trappijintl->getTabsenByIdAbsen($idabsen);
		$this->load->view('transaksi_approval_ijintl/petugas_approval_ijintl',$data);
	...
	public function ajax_proses_approval()
	{
		...
		$this->m_trappijintl->proses_approval_ijintl($data);
	...
	public function ajax_proses_approval_bypetugas()
	{
		...
		$this->m_trappijintl->proses_approval_ijintl_bypetugas($data);
		echo json_encode(array("status" => TRUE));
