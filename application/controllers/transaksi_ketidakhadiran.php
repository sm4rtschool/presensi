...
class Transaksi_ketidakhadiran extends CI_Controller {
	...
	function datatables_pegawai()
	{
		...
		*/
		...
		$aColumns = array('CONVERT(VARCHAR, Tanggal , 120)', 'KdAbsen', 'Keterangan', 'Departemen');
    ...
    $sQuery = "SELECT TOP $limit ".implode(",",$aColumns)."
		...
		$rResult = $this->Trketidakhadiranmodel->getlist_ketidakhadiran($aColumns, $sWhere, $sOrder, $top, $limit, $parameter_login);
       
		$iTotal = 0;
        $rResultTotal = $this->Trketidakhadiranmodel->getlist_ketidakhadiran_total($sIndexColumn, $parameter_login);
        $iTotal = $rResultTotal->num_rows();
		
		//$sQueryCnt = "SELECT * FROM $sTable $sWhere";
		//function getlist_ketidakhadiran_filteredtotal($sIndexColumn, $parameter_login, $aColumns, $sWhere, $sOrder, $top, $limit)
		$iFilteredTotal = 0;
		$rResultTotalFiltered = $this->Trketidakhadiranmodel->getlist_ketidakhadiran_filteredtotal($sIndexColumn, $parameter_login, $aColumns, $sWhere, $sOrder, $top, $limit);
	...
	function list_ketidakhadiran()
	{
		...
		$data['subtitle']=" Transaksi Ketidakhadiran / Pegawai";
		$data['tabletitle']="Table Transaksi Ketidakhadiran";
		...
		$this->load->view('transaksi_ketidakhadiran/list_ketidakhadiran',$data);
	...
	function load_dropdown_alasan()
	{
		...
		$list = $this->Trketidakhadiranmodel->getKdJenisKetidakhadiran();
		$ddata = array();

		foreach ($list as $ex_jenisketidakhadiran) 
		{

			$row = array();

			$row['KdJenisKetidakhadiran'] = $ex_jenisketidakhadiran['KdJenisKetidakhadiran'];
			$row['Jenis'] = $ex_jenisketidakhadiran['Jenis'];
			$row['PersenPotonganTK'] = $ex_jenisketidakhadiran['PersenPotonganTK'];
			$ddata[] = $row;
		
		}

		$output = array(
			"ex_jenisketidakhadiran" => $ddata,
        );
	...
	function GetIsLimit()
	{
		...
		$KdJenisKetidakhadiran = $this->uri->segment(3);
		$list = $this->Trketidakhadiranmodel->GetIsLimit($KdJenisKetidakhadiran);
		$ddata = array();
		...
		foreach ($list as $ex_jenisketidakhadiran) 
		{
			...
			$row = array();
			$row['is_limit'] = $ex_jenisketidakhadiran['is_limit'];
			$ddata[] = $row;
			...
			{  
				...
				foreach ($period as $dt)
				{
					...
					If ($is_limit == 0)
					{
						...
						$array_notif[$xx] = $this->Trketidakhadiranmodel->GenerateAlasanNoLimit($cbopimpinan, $sTanggalAbsen, $data['kdkaryawan'], $cboalasan, $is_eselon, $sKdDepartemen, $sKdJabatan, $sScheduleMasuk, $sSchedulePulang, $iKdShift, $sShift, $memoketerangan, $sSerialNumber, $data['username']);
						//$json = $this->Trketidakhadiranmodel->GenerateAlasanNoLimit($cbopimpinan, $sTanggalAbsen, $data['kdkaryawan'], $cboalasan, $is_eselon, $sKdDepartemen, $sKdJabatan, $sScheduleMasuk, $sSchedulePulang, $iKdShift, $sShift, $memoketerangan, $sSerialNumber, $data['username']);
					
						//$obj = json_decode($json);
						//$is_status_ok = $obj->{'status'};
						//echo $json;
					...
					else //if ($this->input->post('is_limit') = 1)	//  If is_limit = '1' Then  //  jika kdjenisketidakhadiran nya dilimit
					{
						...
						$array_notif[$xx] = $this->Trketidakhadiranmodel->GenerateAlasanLimit($file_element_name, $cbopimpinan, $sTanggalAbsen, $data['kdkaryawan'], $cboalasan, $is_eselon, $sKdDepartemen, $sKdJabatan, $sScheduleMasuk, $sSchedulePulang, $iKdShift, $sShift, $memoketerangan, $sSerialNumber, $data['username']);
				...
				//@unlink($_FILES[$file_element_name]);
					...
					$serror = '<div class="box-body"><div class="alert alert-info alert-dismissable">
					...
					*/
					...
					for ($aa = 1; $aa <= $xx-1; $aa++)
					{
						...
						$sdata_error = $sdata_error . $array_notif[$aa];
						$get_notif_success = $array_notif[$aa];
						
						/*
						If ($get_notif_success == 'success')
						{
							$is_success = 1;
						}
						*/
						
						$json_from_model = $array_notif[$aa];
						$output_json = json_decode($json_from_model);
						$get_status = $output_json->{'status'}; 
						$get_notif = $output_json->{'notif'};
						$get_message = $output_json->{'msg'};
						
						If ($get_status == 'success')
						{
							$data_notif = $data_notif . $get_notif;
						}
						else
						{
							$data_notif = $data_notif . $get_notif . $get_message;
						}						
						
					}
					
					//echo $data_notif;
					$this->session->set_flashdata('notif', $serror . $data_notif . '</div></div>');					
					
					/*
					If ($is_success == 1)
					{
						
						$this->session->set_flashdata('notif', $serror . 'Input data user berhasil.' . '</div></div>');
						
					}
					else
					{
						
						$this->session->set_flashdata('notif', $serror . $sdata_error . '</div></div>');
