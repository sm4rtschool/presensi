...
class M_trappijintl extends CI_Model
{
	...
	function getTabsenByIdAbsen($idabsen)
	{
		...
		$ssql = "SELECT a.*, b.alasan FROM 
				...
				JOIN msalasan_tl b ON a.id_alasan_tl = b.id";
    ...
    function getlist_ijintl($aColumns, $sWhere, $sOrder, $top, $limit, $parameter_login)
    {
		...
		If ($level == 'Pimpinan')
		{
			...
			$ssql = "SELECT * FROM
				(
					SELECT ROW_NUMBER() OVER (ORDER by idabsen DESC) as no_baris, idabsen, is_pimpinan_approve_tl, 
					KdKaryawan, nip, Nama, Tanggal, KdAbsen, Keterangan, Departemen,
					WorkOn, WorkOff, DutyOn, DutyOff, CONVERT(CHAR(5), Late) + ' (' +
					(select KdTerlambatDatang from ex_terlambatdatang WHERE MenitAwal <= x.Late AND x.Late <= MenitAkhir) + ')' AS Late, alasan FROM
					(
										
						SELECT ROW_NUMBER() OVER (ORDER by idabsen) as no_baris, a.idabsen, a.is_pimpinan_approve_tl, a.KdKaryawan, b.nip, b.Nama, 
						a.Tanggal, a.KdAbsen, a.Keterangan, c.Departemen, a.WorkOn, a.WorkOff, a.DutyOn, a.DutyOff, Late, d.alasan
						FROM tabsen a JOIN mskaryawan b ON a.KdKaryawan = b.KdKaryawan
						JOIN msdepartemen c ON a.KdDepartemen = c.RecordID
						JOIN msalasan_tl d ON a.id_alasan_tl = d.id
						WHERE a.kdpimpinan_tl = '$kdkaryawan'
		...
		else if ($level == 'Operator')
		{
			...
			$ssql = "SELECT * FROM
				(
					SELECT ROW_NUMBER() OVER (ORDER by idabsen DESC) as no_baris, idabsen, is_pimpinan_approve_tl, is_petugas_approve_tl, 
					KdKaryawan, nip, Nama, Tanggal, KdAbsen, Keterangan, Departemen,
					WorkOn, WorkOff, DutyOn, DutyOff, CONVERT(CHAR(5), Late) + ' (' +
					(select KdTerlambatDatang from ex_terlambatdatang WHERE MenitAwal <= x.Late AND x.Late <= MenitAkhir) + ')' AS Late, alasan FROM
					(
					
						SELECT ROW_NUMBER() OVER (ORDER by idabsen) as no_baris, a.idabsen, a.is_pimpinan_approve_tl, a.is_petugas_approve_tl, 
						a.KdKaryawan, b.nip, b.Nama, a.Tanggal, a.KdAbsen, a.Keterangan, c.Departemen, a.WorkOn, a.WorkOff, a.DutyOn, a.DutyOff, a.Late, d.alasan
						FROM tabsen a JOIN mskaryawan b ON a.KdKaryawan = b.KdKaryawan
						JOIN msdepartemen c ON a.KdDepartemen = c.RecordID	
						JOIN msalasan_tl d ON a.id_alasan_tl = d.id						
						WHERE a.id_alasan_tl <> 0
	...
	function getlist_ijintl_filteredtotal($sIndexColumn, $parameter_login, $aColumns, $sWhere, $sOrder, $top, $limit)
	{
		...
		If ($level == 'Pimpinan')
		{
			...
			$ssql = "SELECT $sIndexColumn FROM
					(
						SELECT no_baris, idabsen, is_pimpinan_approve_tl, 
						KdKaryawan, nip, Nama, Tanggal, KdAbsen, Keterangan, Departemen,
						WorkOn, WorkOff, DutyOn, DutyOff, Late FROM
							(
							
								SELECT ROW_NUMBER() OVER (ORDER by idabsen) as no_baris, a.idabsen, a.is_pimpinan_approve_tl, a.KdKaryawan, b.nip, b.Nama, 
								a.Tanggal, a.KdAbsen, a.Keterangan, c.Departemen, a.WorkOn, a.WorkOff, a.DutyOn, a.DutyOff, Late
								FROM tabsen a JOIN mskaryawan b ON a.KdKaryawan = b.KdKaryawan
								JOIN msdepartemen c ON a.KdDepartemen = c.RecordID
		...
		else if ($level == 'Operator')
		{
			...
			$ssql = "SELECT $sIndexColumn FROM
					(
						SELECT no_baris, idabsen, is_petugas_approve_tl, nip, Nama, Tanggal, KdAbsen, Keterangan, Departemen FROM
							(
								SELECT ROW_NUMBER() OVER (ORDER by idabsen) as no_baris, a.idabsen, a.is_petugas_approve_tl, 
...
";
	...
	function proses_approval_ijintl($data)
	{
		$is_approve = $data['is_approve'];
		$idabsen = $data['idabsen'];
		$ssql = "UPDATE tabsen SET is_pimpinan_approve_tl = '$is_approve' WHERE idabsen = '$idabsen'"; 
		return $this->db->query($ssql);
		
	}
	
	function proses_approval_ijintl_bypetugas($data)
	{
		$is_approve = $data['is_approve'];
		$idabsen = $data['idabsen'];
		$kdoperator = $data['kdoperator'];
		$ssql = "UPDATE tabsen SET is_petugas_approve_tl = '$is_approve', is_tl_finish = 1, kdpetugas_tl = '$kdoperator' WHERE idabsen = '$idabsen'"; 
		return $this->db->query($ssql);
