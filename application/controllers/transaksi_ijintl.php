...
class Transaksi_ijintl extends CI_Controller {
	...
	function list_ijintl()
	{
		...
		$data['subtitle']=" Transaksi Ijin Terlambat Datang / Pegawai";
		...
		$this->load->view('transaksi_ijintl/list_ijintl',$data);
		...
		foreach ($rResult->result() as $aRow) 
		{
			...
			//$row[] = $numbering + $page; 
				...
				if ($is_pimpinan_approve_tl == 0)	
				{
					...
					$row[] = '<i class="ui-tooltip fa fa-check-square-o" title="Proses" style="font-size: 22px;color:#aa2222; cursor:pointer;" data-original-title="Proses" onclick="konfirmasi_tl('."'".$aRow->idabsen."'".')"></i>';
	...
	function add_ijintl()
	{
		...
		else
		{
			...
			if($this->trijintlmodel->getTabsenRequestByIdAbsen($idabsen)->num_rows()>0)
			{
				
				$data['tabsen'] =  $this->trijintlmodel->getTabsenRequestByIdAbsen($idabsen);
				$data['is_posting_data'] = 0;
				$data['mspimpinan'] = $this->trijintlmodel->getPimpinan();
	...
	function save_add()
	{
		...
		else
		{
			...
			{  
				...
				$path_attachment_tl = $this->config->item('path_attachment_tl');
				
				$notif = $this->trijintlmodel->save($path_attachment_tl, $file_element_name, $cbopimpinan, 0, 0, 0, $cboalasan, $idabsen);				
				
				$this->session->set_flashdata('notif', '<div class="box-body"><div class="alert alert-info alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                    <h4><i class="icon fa fa-info"></i> Info!</h4>
                    Konfirmasi terlambat datang berhasil.
                  </div></div>');
	...
	function load_dropdown_alasan()
	{
		...
		$list = $this->trijintlmodel->getAlasanTL();
		$ddata = array();

		foreach ($list as $msalasan_tl) 
		{

			$row = array();

			$row['id_alasan'] = $msalasan_tl['id'];
			$row['alasan'] = $msalasan_tl['alasan'];
			$ddata[] = $row;
	...
	function detail_absen()
	{
		...
		$idabsen = $this->uri->segment(3);
		$data['list'] = $this->trijintlmodel->getBiodataMskaryawanByIdAbsen($idabsen, $level_user);
		$data['tabsen'] = $this->trijintlmodel->getTabsenByIdAbsen($idabsen);
		
		$this->load->view('transaksi_ijintl/detail_ijintl',$data);
