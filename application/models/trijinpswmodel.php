...
class Trijinpswmodel extends CI_Model{
	...
	function getAlasanPSW()
	{
		
		$ssql = "SELECT * FROM msalasan_psw ORDER BY id ASC";
		$query = $this->db->query($ssql);
	...
	function getTabsenByIdAbsen($idabsen)
	{
		...
		$ssql = "SELECT a.*, b.alasan FROM tabsen a LEFT JOIN
				(
					SELECT * FROM msalasan_psw
				) b
				ON a.id_alasan_psw = b.id WHERE idabsen = '$idabsen' ";
...
*/
	...
	function getlist_ijinpsw($aColumns, $sWhere, $sOrder, $top, $limit, $parameter_login)
    {
		...
		$ssql = "SELECT * FROM
			...
			Tanggal, Keterangan, WorkOn, WorkOff, DutyOn, DutyOff, nip, Nama, is_pimpinan_approve_psw, is_petugas_approve_psw, alasan, CONVERT(CHAR(5), HomeEarly) + ' (' +
			(select KdPsw from ex_psw WHERE MenitAwal <= x.HomeEarly AND x.HomeEarly <= MenitAkhir) + ')' AS HomeEarly FROM
			(
				SELECT a.idabsen, a.Tanggal, a.Keterangan, a.WorkOn, a.WorkOff, a.DutyOn, a.DutyOff, a.HomeEarly, b.nip, b.Nama, is_pimpinan_approve_psw, is_petugas_approve_psw, c.alasan
				FROM tabsen a 
				JOIN mskaryawan b ON a.KdKaryawan = b.KdKaryawan	
				LEFT JOIN msalasan_psw c ON a.id_alasan_psw = c.id 	
				WHERE a.KdKaryawan = '$kdkaryawan' AND HomeEarly > 0 
			) x
			$sWhere
			) y
	...
	function getlist_ijinpsw_filteredtotal($sIndexColumn, $parameter_login, $aColumns, $sWhere, $sOrder, $top, $limit)
	{
        ...
        $query = $this->db->query("
			...
			(
			SELECT ROW_NUMBER() OVER (ORDER by idabsen DESC) as no_baris, idabsen, Tanggal, Keterangan, DutyOn, DutyOff, HomeEarly, nip, Nama FROM
			(
				SELECT a.idabsen, a.Tanggal, a.Keterangan, a.DutyOn, a.DutyOff, a.HomeEarly, b.nip, b.Nama
				FROM tabsen a 
				JOIN mskaryawan b ON a.KdKaryawan = b.KdKaryawan			
				WHERE a.KdKaryawan = '$kdkaryawan' AND HomeEarly > 0
			) x
    ...
    function getlist_ijinpsw_total($sIndexColumn, $parameter_login)
	{
        ...
        $query = $this->db->query("SELECT COUNT(*) AS total_jml_data FROM tabsen WHERE KdKaryawan = '$kdkaryawan' AND HomeEarly > 0");
		$total_jml_data = $query->row()->total_jml_data;  
		return $total_jml_data;
		
    }
	
	function save($path_attachment_psw, $file_element_name, $kdpimpinan_psw, $is_pimpinan_approve_psw, $is_petugas_approve_psw, $is_psw_finish, $id_alasan_psw, $idabsen)
	{
		...
		if (empty($_POST['cboalasan']))
		{
			...
			$msg = "Please enter a title";
				...
				if ($status != "error")
				{
					...
					$config['upload_path'] = $path_attachment_psw;
					...
					{					
						...
						$nama_file = 'psw' . $idabsen . $data['file_ext']; 
						$type_file = $data['file_type'];
						
						$ssql = "UPDATE tabsen SET kdpimpinan_psw = '$kdpimpinan_psw', is_pimpinan_approve_psw = '$is_pimpinan_approve_psw', 
						is_petugas_approve_psw = '$is_petugas_approve_psw', is_psw_finish = '$is_psw_finish', id_alasan_psw = '$id_alasan_psw', 
						nama_file_psw = '$nama_file', type_file_psw = '$type_file'
...
JOIN
(
SELECT xx.KdKaryawan_mskaryawan, xx.nama AS nama_pegawai, xx.departemen, xx.jabatan, xx.kdpimpinan_psw, xx.kdpetugas_psw, yy.Nama as nama_pimpinan 
FROM (
				...
				JOIN
				(
					SELECT KdKaryawan as KdKaryawan_tabsen, kdpimpinan_psw, kdpetugas_psw FROM tabsen WHERE idabsen = $idabsen
				)
				b
				ON a.KdKaryawan_mskaryawan = b.KdKaryawan_tabsen
) xx
...
ON aa.KdKaryawan = bb.kdpetugas_psw
";
			...
			else
			{
				...
				$ssql = "SELECT xx.KdKaryawan_mskaryawan, xx.nama AS nama_pegawai, xx.departemen, xx.jabatan, xx.kdpimpinan_psw, xx.kdpetugas_psw, yy.Nama as nama_pimpinan 
FROM (
